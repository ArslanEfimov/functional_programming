...
-export([test/0, remove/2, insert/2, foldl/3, foldr/3, filter/2, map/2, new/1, new/0, get_element/2, has_key/2, union/2, to_list/1]).
-include("records.hrl").
...
insert(Key, Set = #hash_set{size = Size, capacity = Capacity, table = Table}) ->
...
remove(Key, Set = #hash_set{size = Size, capacity = Capacity, table = Table}) ->
  Index = hash(Key, Capacity),
...
resize_and_insert(Key, Set = #hash_set{capacity = Capacity}) ->
  NewCapacity = Capacity * 2,
  ResizedSet = new(NewCapacity),
  ResizedSetUpdated = foldl(fun(K, Acc) ->
    case K =:= undefined of
      true -> Acc;
      false -> insert(K, Acc)
    end end, ResizedSet, Set),
  insert(Key, ResizedSetUpdated).


filter(Pred, Set = #hash_set{table = Table}) ->
...
map(Fun, Set = #hash_set{capacity = Capacity}) ->
  NewSet = new(Capacity),
  foldl(
    fun(Key, AccSet) ->
...
union(Set1 = #hash_set{size = Size1, capacity = Capacity1},
    Set2 = #hash_set{size = Size2, capacity = Capacity2}) ->
...
to_list_helper([], Acc) ->
  lists:reverse(Acc);  % Возвращаем список в обратном порядке
to_list_helper([undefined | T], Acc) ->
  to_list_helper(T, Acc);  % Пропускаем элементы, равные undefined
to_list_helper([H | T], Acc) ->
  to_list_helper(T, [H | Acc]).  % Добавляем элемент в аккумулятор
