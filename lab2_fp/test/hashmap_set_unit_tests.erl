...
map_set_test() ->
  ...
  ?assert(hashmap_set:has_key(Set5, 9)),   % 3^2 = 9
  ?assert(hashmap_set:has_key(Set5, 49)),  % 7^2 = 49
  ?assert(hashmap_set:has_key(Set5, 484)), % 22^2 = 484
  ?assert(hashmap_set:has_key(Set5, 225)). % 15,2 = 225
...
union_set_test() ->
  ...
  Set1Updated = lists:foldl(fun(Elem, AccSet) ->
    hashmap_set:insert(Elem, AccSet)
                            end, Set1, lists:seq(1, 500)),

  Set2Updated = lists:foldl(fun(Elem, AccSet) ->
    hashmap_set:insert(Elem, AccSet)
                            end, Set2, lists:seq(501, 1000)),
