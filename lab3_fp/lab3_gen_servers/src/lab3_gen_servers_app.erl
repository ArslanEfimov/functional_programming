%%%-------------------------------------------------------------------
%% @doc lab3_gen_servers public API
%% @end
%%%-------------------------------------------------------------------
...
start() ->
    ...
    io:fwrite("Пользователь задал частоту дискретизации - ~p, окна - ~p, методы интерполяции - ~p~n", [
        Freq, Window, Methods
    ]),
    ...
    case string:to_float(Num) of
        {error, _} ->
            case string:to_integer(Num) of
                {error, _} -> io:fwrite("Ошибка в попытке запарсить аргументы");
