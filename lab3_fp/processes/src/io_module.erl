  ...
  after 100 ->
    ...
    case parse_points(string:trim(io:get_line(""))) of
      {ok, [X, Y]} ->
        ...
        input_loop();
      {error, _} ->
        io:fwrite("Некорректный ввод. Формат: X Y\n"),
        input_loop()
    end
  end.
...
output_loop() ->
  receive
    {ok, Method, Result} when Method =:= linear; Method =:= lagrange ->
      io:fwrite("\nРезультаты интерполяции метода (~s):\n", [erlang:atom_to_list(Method)]),
      print_result(Result),
      output_loop();
    {error, Method, Msg} ->
      io:fwrite("\e[31mОшибка в методе ~s: ~s\e[0m\n",
        [erlang:atom_to_list(Method), Msg]),
      output_loop()
  end.

print_result([GeneratedDots, InterpolatedValues]) ->
  io:fwrite("Сгенерированные точки:\n"),
  lists:foreach(
