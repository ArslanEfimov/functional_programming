...
start() ->
  ...
  io:fwrite("Пользователь задал частоту дискретизации - ~p, окна - ~p, методы интерполяции - ~p~n", [Freq, Window, Methods]),
  ...
  receive
    {'EXIT', From, Reason} ->
      case Reason of
        ...
        _ ->
          io:fwrite(
            "Дочерний процесс ~p остановился по причине: ~p~n",
            [From, Reason]
          )
      end;
    _Other ->
  ...
  case string:to_float(Num) of
    {error, _} ->
      case string:to_integer(Num) of
        {error, _} -> io:fwrite("Ошибка парсинга числа");
        Int -> element(1, Int)
      end;
    Float ->
      element(1, Float)
